...
export const APIProvider = ({children}) =>
    {
        ...
        const getKategoriak = async () => {
            try {
                const response = await myAxios.get("http://localhost:8000/api/categories");
                console.log(response.data)
                setKL(response.data)
            } catch (error) {
                console.error("Hiba:", error);
        ...
        return ( <APIContext.Provider value={{sajatVersenyLista, helyszinLista, categoryLista}}>{children}</APIContext.Provider> )
    }
